...
class User {
  static async getAll() {
    console.log("GetAll accessed " + Date.now());
    try {
  ...
  static async create(userDetails) {
    ...
    try {
      ...
      const results = await pool.query(Query.registerUserQuery, [
        username,
        hashPassword,
        email,
        full_name,
  ...
  static async login(userDetails) {
    ...
    try {
      ...
      if (results.rowCount === 0) {
        ...
        const isPasswordCorrect = this.#isPasswordCorrect(
          password,
  ...
  static async changePW(id, newPassword, userDetails) {
    ...
    try {
      ...
      if (id != userDetails.id) {
        ...
        const isPasswordSame = this.#isPasswordCorrect(
          newPassword,
          result.rows[0].password
        );
        if (isPasswordSame === false) {
          const hashPassword = this.#hashPassword(newPassword, 10);
          const updateResult = await pool.query(Query.changePasswordQuery, [
            hashPassword,
            id,
          ]);
